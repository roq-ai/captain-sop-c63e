...
import { getExperts, deleteExpertById } from 'apiSdk/experts';
import { ExpertInterface } from 'interfaces/expert';

type ColumnType = ColumnDef<ExpertInterface, unknown>;
...
export function ExpertListPage(props: ExpertListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ExpertInterface>>(
    () => `/experts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ExpertInterface) => {
    if (hasAccess('expert', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/experts/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/experts/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('expert', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/experts/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('expert', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="expert" />
        </Flex>

        {hasAccess('expert', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/experts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
