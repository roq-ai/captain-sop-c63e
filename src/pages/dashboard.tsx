...
import { getExperts } from 'apiSdk/experts';
import { ExpertInterface } from 'interfaces/expert';
import { getSops } from 'apiSdk/sops';
import { SopInterface } from 'interfaces/sop';

import { SopListPage as ShowCaseList } from 'pages/sops';

function DashboardPage() {
  const { session } = useSession();

  const expertStat = useSWR<PaginatedInterface<ExpertInterface>>(
    () => `/experts`,
    () => getExperts({ limit: 1, offset: 0 }),
  );

  const sopStat = useSWR<PaginatedInterface<SopInterface>>(
    () => `/sops`,
    () => getSops({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Experts',
      result: expertStat?.data?.totalCount,
      isLoading: expertStat.isLoading,
    },
    {
      title: 'Sops',
      result: sopStat?.data?.totalCount,
      isLoading: sopStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`sops`} passHref legacyBehavior>
                  <Button
